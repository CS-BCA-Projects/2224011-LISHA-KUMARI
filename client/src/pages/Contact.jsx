...
const Contact = () => {
  ...
  useEffect(() => {
    const fetchMessages = async () => {
      try {
        const response = await axios.get(`${backendUrl}/api/contact/messages/${userId}`, {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (response.status === 201) {
        ...
        const updatedMessages = await axios.get(`${backendUrl}/api/contact/messages/${userId}`, {
          withCredentials: true,
        });
        ...
        {messages.length > 0 ? (
          messages.map((msg) => (
            <div key={msg._id} className="p-4 border border-gray-300 rounded mb-2">
              <p><strong>From:</strong> {msg.senderId?.name || "Unknown"}</p>
